...
public class PopulateDefaultPermTemplateAppColumnOfOrganizations extends DataChange {
  ...
  public void execute(Context context) throws SQLException {
    ...
    massUpdate.select("SELECT o.uuid, o.default_perm_template_project, o.default_perm_template_view FROM organizations o " +
      "WHERE o.default_perm_template_app IS NULL");
    massUpdate.update("UPDATE organizations SET default_perm_template_app=?, updated_at=? WHERE uuid=?");
    massUpdate.execute((row, update) -> {
      String projectDefaultTemplate = row.getString(2);
      String viewDefaultTemplate = row.getNullableString(3);
      update.setString(1, viewDefaultTemplate == null ? projectDefaultTemplate : viewDefaultTemplate);
